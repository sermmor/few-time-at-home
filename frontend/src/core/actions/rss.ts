...
const getRSS = (nameEndpoint: 'mastodon' | 'twitter' | 'blog' | 'youtube', amount: number): Promise<RssDataModel> => 
  fetchJsonReceive<RssDataModel>(queryRssEndpoint(nameEndpoint, amount), rssDataModelMock());

export const RSSActions = { getRSSMasto, getRSSNitter, getRSSBlog, getRSSYoutube, getRSS };
